...
int main()
{
    ...
    // // ####################### matrix declarations
    Eigen::MatrixXd A(2 * listOfFaces.size(), 2 * (listOfPoints.size() - 2));
    ...
    // Eigen::SparseQR<Eigen::MatrixXd, Eigen::COLAMDOrdering<int>> solver;
                   ...
                   Eigen::MatrixXd &RHS)
{
    ...
    // Eigen::MatrixXd A_Mf1_p1, A_Mf1_p2, A_Mf1_p3, A_Mf1_temp, A_Mf1_final;
    // Eigen::MatrixXd A_Mf2_p1, A_Mf2_p2, A_Mf2_p3, A_Mf2_temp, A_Mf2_final;
    ...
    { // loop through the triangles
        ...
        { // loop through the points
            ...
            if (listOfFaces.at(i).get_aIndex() == j) // Weight 1
            {
                ...
                //  real part x3-x2
                //  M(i, j) = 1;
                M(i, j) = 1; // for sparse we need .insert()
                ...
                A_Mf1(i, j) = (Xc - Xb) / sqrt(abs(listOfAreas.at(i)));

                // complex part yc-yb
                A_Mf2(i, j) = (Yc - Yb) / sqrt(abs(listOfAreas.at(i)));
            }
            ...
            if (listOfFaces.at(i).get_bIndex() == j) // Weight 2
            {
                ...
                // real part Xc-Xb   - note these are from wight eauation not top graident in triangle equation
                A_Mf1(i, j) = (Xa - Xc) / sqrt(abs(listOfAreas.at(i)));

                // complex part yc-yb
                A_Mf2(i, j) = (Ya - Yc) / sqrt(abs(listOfAreas.at(i)));
            }
            ...
            if (listOfFaces.at(i).get_cIndex() == j) // Weight 3
            {
                ...
                // real part Xc-Xb   - note these are from wight eauation not top graident in triangle equation
                A_Mf1(i, j) = (Xb - Xa) / sqrt(abs(listOfAreas.at(i)));

                // complex part yc-yb
                A_Mf2(i, j) = (Yb - Ya) / sqrt(abs(listOfAreas.at(i)));
            }
        }
    }
    ...
    */
    ...
    // Eigen::MatrixXd A_top(listOfFaces.size(), 2 * (listOfPoints.size() - 2));
    // Eigen::MatrixXd A_bottom(listOfFaces.size(), 2 * (listOfPoints.size() - 2));
    // Eigen::MatrixXd A(2 * listOfFaces.size(), 2 * (listOfPoints.size() - 2));
    // Eigen::MatrixXd A(listOfFaces.size(), 4 * (listOfPoints.size() - 2)); /// wrong, if concatting verticallly
